...
local SPATIAL_GLIMPSE_LOCATION_SIZE = 3
--[[data]]--
...
locationSensor:add(nn.Linear(SPATIAL_GLIMPSE_LOCATION_SIZE, opt.locatorHiddenSize))
...
glimpseSensor:add(nn.ScaledSpatialGlimpse(opt.glimpsePatchSize, opt.glimpseDepth):float())
...
glimpseSensor2:add(nn.ScaledSpatialGlimpse(opt.glimpsePatchSize, opt.glimpseDepth):float())
...
locator:add(nn.Linear(opt.hiddenSize, SPATIAL_GLIMPSE_LOCATION_SIZE))
...
agent = nn.Sequential()
agent:add(nn.Convert(ds:ioShapes(), 'bpchw'))
agent:add(attention)
